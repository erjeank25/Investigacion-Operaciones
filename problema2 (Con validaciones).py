...
def main():
    try:
        ...
        if almacenes <= 0:
            raise ValueError("El número de almacenes debe ser mayor que cero.")
        
        centros = int(input("Introduce el número de centros de distribución: "))
        if centros <= 0:
            raise ValueError("El número de centros de distribución debe ser mayor que cero.")
        
        ofertas = []
        for i in range(almacenes):
            oferta = int(input(f"Introduce la oferta del almacén {i + 1}: "))
            if oferta < 0:
        ...
        for j in range(centros):
            demanda = int(input(f"Introduce la demanda del centro de distribución {j + 1}: "))
            if demanda < 0:
        ...
        for i in range(almacenes):
            fila_costos = []
            for j in range(centros):
                costo = float(input(f"Introduce el costo de transporte del almacén {i + 1} al centro de distribución {j + 1}: "))
                if costo < 0:
